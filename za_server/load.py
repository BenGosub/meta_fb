import facepy
import sqlalchemy
from models import Profile_Likes, Comment, Post, Profile, engine
import datetime
...
page_ids = session.query(Profile.id).all()
page_ids = [id for (id, ) in page_ids]

with open('/home/uanalyti/meta_fb/at', 'r') as f:
    at = f.readline()
...
def get_since_timestamp(page_id):
    most_recent_post_time = session.query(Post).filter_by(page_id=page_id).order_by \
    (Post.post_created_time.desc()).all()[0].post_created_time
    most_recent_post_time = time.mktime(most_recent_post_time.timetuple())
    return most_recent_post_time

def convert_to_datetime(created_at):
    created_at = dateutil.parser.parse(created_at)
    created_at = created_at.replace(tzinfo=datetime.timezone.utc).astimezone(tzutc())
    return created_at
